...
type UserOrganization struct {
	UserID         uint      `json:"user_id"         gorm:"not null;"`               // 用户 ID，不能为空，索引
	OrganizationID uint      `json:"organization_id" gorm:"not null;"`               // 组织 ID，不能为空，索引
	...
	IsLeaved       int       `json:"is_leaved"       gorm:"column:is_leaved;type:int"`
}
